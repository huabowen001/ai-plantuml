...
return g.yield(fetch(window.location.origin+"/plantuml/coder",{method:"POST",headers:{"Content-Type":"text/plain"},body:a}),4);case 4:c=g.yieldResult;if(!c.ok){handlePreviewError();g.jumpTo(5);break}return g.yield(c.text(),6);case 6:e=g.yieldResult,d=window.location.origin+"/plantuml/svg/"+e,b.innerHTML='\x3cimg src\x3d"'+d+'" alt\x3d"PlantUML Diagram" onerror\x3d"handlePreviewError()"\x3e';case 5:g.leaveTryBlock(0);break;case 2:f=g.enterCatchBlock(),console.error("Preview error:",f),handlePreviewError(),
g.jumpToEnd()}})}function handlePreviewError(){document.getElementById("previewContainer").innerHTML='\x3cp class\x3d"preview-placeholder" style\x3d"color: var(--error-color);"\x3e\u9884\u89c8\u751f\u6210\u5931\u8d25\uff0c\u8bf7\u68c0\u67e5PlantUML\u4ee3\u7801\x3c/p\x3e'}
...
e);case 3:return d.return(encodeURIComponent(a))}})}function formatDate(a){a=new Date(a);var b=new Date-a;return 6E4>b?"\u521a\u521a":36E5>b?Math.floor(b/6E4)+"\u5206\u949f\u524d":864E5>b?Math.floor(b/36E5)+"\u5c0f\u65f6\u524d":6048E5>b?Math.floor(b/864E5)+"\u5929\u524d":a.toLocaleDateString("zh-CN")}
